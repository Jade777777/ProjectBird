...
namespace Core.Managers.Analytics
{
    public class SessionAnalyticsData
    {
        ...
        public TimeSpan GetGameTimeAfterChicksFed()
        {
            var chicksFedEvent = (AllChicksFedEvent)CapturedEvents.Find(@event => @event.GetType() == typeof(AllChicksFedEvent));
            if (chicksFedEvent != null)
                return EndDateTime - chicksFedEvent.time;
